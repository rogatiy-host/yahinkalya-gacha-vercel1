...
router.post('/cards', upload.single('image'), async (req, res, next) => {
  try {
    const imageUrl = req.file ? `/uploads/cards/${req.file.filename}` : req.body.image_url;
    if (!imageUrl) throw new Error('Нужно изображение карты');
...
router.patch('/cards/:id', upload.single('image'), async (req, res, next) => {
  try {
    ...
    const imageUrl = req.file ? `/uploads/cards/${req.file.filename}` : (req.body.image_url || current.image_url);
    const card = await one(`
