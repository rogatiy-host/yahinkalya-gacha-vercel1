...
-- ===========================================================================
-- Доп. миграции (безопасно накатывать повторно на уже существующую базу —
-- всё через IF NOT EXISTS / ADD COLUMN IF NOT EXISTS).
-- ===========================================================================

-- Теги на карточках, чтобы баннер мог бустить не только целую серию, а любой набор карт.
...
-- Баннер может явно бустить набор тегов, а не только одну серию.
ALTER TABLE banners ADD COLUMN IF NOT EXISTS tags TEXT[] NOT NULL DEFAULT '{}';

-- Конкретные карты, "прибитые" к баннеру (показываются в его витрине на экране круток
-- и получают тот же буст веса, что и серия/теги).
CREATE TABLE IF NOT EXISTS banner_cards (
  banner_id INTEGER NOT NULL REFERENCES banners(id) ON DELETE CASCADE,
  card_id INTEGER NOT NULL REFERENCES cards(id) ON DELETE CASCADE,
  PRIMARY KEY (banner_id, card_id)
);

-- Красная точка у заявок в друзья и у трейдов: отмечаем, видел ли получатель.
ALTER TABLE friendships ADD COLUMN IF NOT EXISTS seen_by_addressee BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE trades ADD COLUMN IF NOT EXISTS seen_by_to BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE trades ADD COLUMN IF NOT EXISTS seen_by_from BOOLEAN NOT NULL DEFAULT FALSE;

-- Фон шапки профиля — URL картинки одной из уже выбитых карт игрока.
